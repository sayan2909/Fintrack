...
router.post("/", async (req, res) => {
  ...
  if (!user) return unauthorized(res);
  try {
    const { categoryId, category, categoryName, amount, month, description } = req.body ?? {};
    const catName = (category || categoryName || "").trim();
    if (!catName && !categoryId) return fail(res, "Category is required.", 400);
    const amt = parseAmount(amount);
    if (!amt) return fail(res, "Amount must be positive.", 400);
    if (!month || !/^\d{4}-\d{2}$/.test(month)) return fail(res, "Month must be YYYY-MM.", 400);
    ...
    if (dup.length) return fail(res, `A budget for ${finalName} already exists for ${month}.`, 409);
    ...
    return ok(res, { budget: { ...rows[0], spent: 0, remaining: amt, percentUsed: 0, status: "healthy" } }, 201);
  } catch (e) {
    console.error("create budget", e);
    return fail(res, "Unable to create budget.", 500);
  }
});
...
router.put("/:id", async (req, res) => {
  ...
  if (!rows[0]) return notFound(res, "Budget not found.");
  try {
    const body = req.body;
