...
router.get("/", async (req, res) => {
  ...
  const preset = (req.query.preset as string) || "monthly";
  let from: string;
  let to: string;
  const now = new Date();
  ...
  if (preset === "quarterly") {
    ...
    from = (req.query.from as string) || `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-01`;
    to = (req.query.to as string) || now.toISOString().slice(0, 10);
  } else {
    from = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-01`;
    to = now.toISOString().slice(0, 10);
  }
  if (from > to) return fail(res, "Invalid date range.", 400);
  ...
  if (req.query.format === "csv") {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="fintrack-report-${from}-to-${to}.csv"`);
