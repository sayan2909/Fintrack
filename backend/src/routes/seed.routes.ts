...
router.post("/", async (req, res) => {
  ...
  let account = (await db.select().from(accounts).where(eq(accounts.userId, user.id)).limit(1))[0];
  if (!account) {
  ...
  if (has.length === 0) {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      if (i % 30 === 5) {
        ...
        txRows.push({ userId: user.id, accountId: account?.id, categoryId: sal?.id, categoryName: "Salary", type: "income", amount: "85000", description: "Monthly salary", date: d.toISOString().slice(0, 10), paymentMethod: "Bank Transfer" });
      }
      if (i % 17 === 3) {
        const fr = catByName("Freelance");
        txRows.push({ userId: user.id, accountId: account?.id, categoryId: fr?.id, categoryName: "Freelance", type: "income", amount: String(5000 + Math.floor(Math.random() * 15000)), description: "Freelance project", date: d.toISOString().slice(0, 10), paymentMethod: "Bank Transfer" });
      }
    }
  ...
  return ok(res, { seeded: true, email: demoEmail, password: "Demo@1234" });
});
