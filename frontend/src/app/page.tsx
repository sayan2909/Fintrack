      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-4 pb-16 pt-14 sm:px-6 lg:grid-cols-2 lg:pt-20">
          ...
          <div className="animate-fade-up relative" style={{ animationDelay: "0.15s" }}>
            <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xl dark:border-slate-800/80 dark:bg-[#111827] transition-colors">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Total Balance</p>
                  <p className="text-3xl font-extrabold text-slate-900 dark:text-white">₹1,25,400</p>
                  <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">▲ 12.4% this month</p>
                </div>
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-600 text-white shadow-sm shadow-indigo-500/25">
                  <Wallet className="h-5 w-5" />
                </div>
              </div>
              <div className="mt-4 grid grid-cols-3 gap-3">
                {[["Income", "₹85,000", "text-emerald-600 dark:text-emerald-400"], ["Expenses", "₹42,300", "text-rose-500 dark:text-rose-400"], ["Savings", "43.2%", "text-indigo-600 dark:text-indigo-400"]].map(([l, v, c]) => (
                  <div key={l} className="rounded-xl bg-slate-50 p-3 border border-slate-100 dark:bg-slate-900/60 dark:border-slate-800/60">
                    <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400">{l}</p>
              ...
              <div className="mt-4 space-y-2.5">
                {[["Food", 80, "bg-amber-500", "₹4,000 / ₹5,000"], ["Shopping", 55, "bg-pink-500", "₹4,400 / ₹8,000"], ["Travel", 30, "bg-teal-500", "₹1,800 / ₹6,000"]].map(([n, p, bar, sub]) => (
                  <div key={n as string}>
                    <div className="mb-1 flex justify-between text-xs font-semibold text-slate-800 dark:text-slate-200">
                      <span>{n}</span>
            ...
            <div className="absolute -right-3 -top-3 hidden rounded-xl border border-slate-200 bg-white px-4 py-2.5 shadow-lg sm:block dark:border-slate-700 dark:bg-[#111827]">
              <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400">Savings rate up!</p>
              <p className="text-[11px] text-slate-500 dark:text-slate-400">+6.2% vs last month</p>
            </div>
          </div>
        </div>
      </section>
